...
const COLUMNS: { id: TaskStatus; label: string; color: string; badge: string }[] = [
  { id: "todo",        label: "To Do",        color: "border-gray-300",   badge: "bg-gray-100 text-gray-600" },
  { id: "in_progress", label: "Em Progresso",  color: "border-blue-400",   badge: "bg-blue-100 text-blue-700" },
  { id: "review",      label: "Review",        color: "border-amber-400",  badge: "bg-amber-100 text-amber-700" },
  { id: "done",        label: "Concluído",     color: "border-emerald-400", badge: "bg-emerald-100 text-emerald-700" },
];
...
function TaskCard({
  ...
  return (
    <div
      ...
      style={style}
      {...attributes}
      className={`bg-white rounded-xl border border-gray-100 p-3 shadow-sm group
...
export default function SprintsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    if (sprintData && sprintData.length > 0) {
      ...
      const current =
        sprintData.find((s: Sprint) => s.status === "active") ?? sprintData[0];
  ...
  async function createSprint(name: string, goal: string) {
    const { data } = await supabase
      .from("sprints")
      .insert({ name, goal, status: "active" })
      .select()
      .single();
    if (data) {
          ...
          {sprintDropdown && (
            <>
              ...
              <div className="absolute top-full left-0 mt-1 w-56 bg-white border border-gray-100 rounded-xl shadow-lg z-20 py-1 overflow-hidden">
                ...
                <button
                  onClick={() => { setSprintDropdown(false); setShowNewSprint(true); }}
      ...
      {activeSprint ? (
        <div className="flex-1 overflow-x-auto p-6">
          ...
          >
            ...
            <DragOverlay>
              {draggingTask && (
                <TaskCard task={draggingTask} overlay onDelete={() => {}} />
              )}
            </DragOverlay>
          </DndContext>
        </div>
